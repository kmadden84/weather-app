...
export class App extends Component {
  ...
  async fetchCurrentWeather(city = "") {
    ...
    const fetchedData = await fetch(`https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&appid=6b5196540a620aafdc0de6bed60c4584&units=metric`);
  ...
  async fetchCityForecast(city = "") {
    const [lat, lon] = returnCoordinates(city);
    const fetchedData = await fetch(`https://api.openweathermap.org/data/2.5/forecast?lat=${lat}&lon=${lon}&appid=6b5196540a620aafdc0de6bed60c4584&units=metric`);
    const parsedData = await fetchedData.json()
    return parsedData;
  }
